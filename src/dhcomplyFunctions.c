    ...
    for (int i = 0; i < request->option_count; i++) {
        ...
        switch (option_code) {
            case IA_NA_OPTION_CODE:
                ...
                char cmd[512];
                sprintf(cmd, "ip -6 addr %llu/%d dev %s preferred_lft %lu valid_lft %lu", request->option_list[option_index].ia_address_t.ipv6_address, 128, iface, request->option_list[option_index].ia_address_t.prefered_lifetime, request->option_list[option_index].ia_address_t.valid_lifetime);
