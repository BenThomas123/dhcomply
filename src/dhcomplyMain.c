...
int main(int argc, char *argv[]) {
    ...
    if (!isStatless) {
        ...
        while (true) {
            restartStateless:
            dhcpv6_message_t *firstInfoReq = buildInformationRequest(config_file, argv[2]);
            sendInformationRequest(firstInfoReq, sockfd, argv[2], 0);

            uint8_t *reply_packet = (uint8_t *)calloc(MAX_PACKET_SIZE, sizeof(uint8_t));
            int reply_check = check_for_message(sockfd, reply_packet, REPLY_MESSAGE_TYPE);
            dhcpv6_message_t *reply_message = parseStatelessReply(reply_packet, firstInfoReq, argv[2], reply_check);
            int index = get_option_index(reply_packet, reply_check, INFORMATION_REFRESH_OPTION_CODE);
            if (index != -1) {
                refresh_time = reply_message->option_list[index].information_refresh_time_t.information_refresh_time * MILLISECONDS_IN_SECONDS;
            }
            index = get_option_index(reply_packet, reply_check, INF_MAX_RT_OPTION_CODE);
            if (index != -1) {
                inf_max_rt = reply_message->option_list[index].INF_MAX_RT_t.INF_MAX_RT_value * MILLISECONDS_IN_SECONDS;
            }
            if (reply_check && reply_message->valid) {
                usleep(refresh_time * MILLISECONDS_IN_SECONDS);
            ...
            while (retransmissionInfoReq < INFO_REQUEST_RETRANS_COUNT) {
                uint64_t retrans_time = lower_solicit[retransmissionInfoReq] + (rand() % (upper_solicit[retransmissionInfoReq] - lower_solicit[retransmissionInfoReq]));
                ...
                if (reply_check && reply_message->valid) {
                    ...
                    usleep(retrans_time * MILLISECONDS_IN_SECONDS);
                    if (elapse_time < 655350) {
                        sendInformationRequest(firstInfoReq, sockfd, argv[2], elapse_time / 10);
                    } else {
                        sendInformationRequest(firstInfoReq, sockfd, argv[2], 65535);
                    }
